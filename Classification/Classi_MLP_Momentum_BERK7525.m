...
bestEff = zeros(1,1);
weight_in = zeros(100,391,4);
weight_out = zeros(11,100,4);
errors = zeros(4,1);
inp_rows = 1:99;
out_rows = 1:99;
set=1;
...
%             Wo = Wo + DWo;
    ...
    for i=1:size(val_conf,1)
        fprintf('Individual Efficiency for all the classes are given as follows : Class %d is %f',i,individual_efficiency(i,1));
        fprintf('\n');
    end
    fprintf('\n');
...
%end
fprintf('Efficiency = %d',max(bestEff(:,1)));
bestEff
...
save -ascii mlpMomentum_BERK19.dat op;